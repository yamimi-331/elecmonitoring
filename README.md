...
### 👨‍💻 담당 역할
  ...
  * **Spring 기반 전체 웹 개발 구조 설계 및 구현**
  * **로그인/권한 관리, A/S 신고 및 처리 기능 개발**
  * **Python FastAPI 연동** (설비 상태 데이터 분석 연계)
  * **MySQL DB 설계** (노후시설, 신고 이력, 사용자, 권한 테이블)
  * **Chart.js 기반 상태 모니터링 대시보드 개발**
  * **관리자/직원 전용 페이지 권한별 메뉴 분리**
...
## 🤝 협업 및 커뮤니케이션
  ...
  * **GitHub**로 버전 관리 및 공동 브랜치 관리
  * **Notion, Excel**로 기능 정의서 작성 및 진행 관리
  * 파트너와 DB·백엔드·프론트 파트를 나눠 분업, 로직 설계 주도
...
(이곳에 모니터링 대시보드 화면 스크린샷 또는 GIF 삽입)

### 🗂️ A/S 처리 현황 및 캘린더

(이곳에 A/S 처리 현황 및 캘린더 화면 스크린샷 또는 GIF 삽입)

### ✅ 권한별 메뉴 및 페이지 접근 제어

(이곳에 권한별 메뉴 및 페이지 접근 제어 화면 스크린샷 또는 GIF 삽입)
