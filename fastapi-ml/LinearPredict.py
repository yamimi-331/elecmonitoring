...
def predict(region='서울특별시', type_='전기화재', periods=3, plot=True):
    # 모델 및 인코더 로드
    model = joblib.load('./models/linear_fire_multioutput_model.pkl')
    le_region = joblib.load('./models/linear_le_region.pkl')
...
#     plt.title(f"{region} 전기화재 피해액 및 피해건수 실측 vs 예측 (Linear Regression)")
