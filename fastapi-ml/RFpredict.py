...
def predict_by_region(region='서울특별시', type_='전기화재', periods=3):
    safe_name = safe_region_name(region)
    model_path = f'./models/by_region/rf_model_{safe_name}.pkl'
    ...
    X_actual = pd.DataFrame({
        'year_from_start': [y - min_year for y in years_actual]
    })
    ...
    if periods > 0:
        X_future = pd.DataFrame({
            'year_from_start': [y - min_year for y in future_years]
        })
...
#     region = "서울특별시"
#     periods = 3
#     results = predict_by_region(region, '전기화재', periods)
...
#     plt.title(f"{region} 전기화재 피해액 및 피해건수 실측 vs 예측 (랜덤포레스트)")
